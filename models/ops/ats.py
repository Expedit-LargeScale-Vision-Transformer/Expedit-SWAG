...
def inverse_transform_sampling(
    ...
    attn_tmp = torch.gather(
        ...
        .unsqueeze(3)
        .expand(B, num_heads, n_tokens - 1, N),
    )
    raw_x_tmp = torch.gather(
...
def _scaled_dot_product_attention(
    ...
    attn = attn.view(B // num_heads, num_heads, Nt, -1)
    attn = softmax_with_policy(attn, policy)    # (B, num_heads, Nt, Ns)
    ...
    v_reshape = v.view(B // num_heads, num_heads, -1, E)
    sorted_scores, sorted_indices = score_assignment_step(attn, v_reshape)

    # --------------------------
    # Inverse Transform Sampling
    # --------------------------

    selected_x, attn, policy, sampler = inverse_transform_sampling(
        sorted_scores, sorted_indices, attn, n_tokens, raw_x, n_ref_tokens, num_heads
    )

    attn = attn.view(B, Nt, -1)
    ...
    if drop_tokens:
        C = E * num_heads
        output = output.reshape(B // num_heads, num_heads, Nt, E).transpose(1, 2).reshape(-1, Nt, C)
        ...
        out_zero_mask = torch.zeros(B // num_heads * out_mask_size, C, 
            dtype=x_prunned.dtype, device=x_prunned.device)
        # out_zero_mask = torch.zeros_like(x_prunned)
        # out_zero_mask = self.out_zero_mask.expand(B * out_mask_size, -1)

        output = out_zero_mask.scatter(
            0, sampler_output, x_prunned, reduce="add"
        ).reshape((-1, out_mask_size, C))
        ...
        policy = (
            ...
            .reshape(-1, out_mask_size, 1)
        )
        output = output.reshape(B // num_heads, out_mask_size, num_heads, E).transpose(1, 2).reshape(-1, out_mask_size, E)

    return output, attn, selected_x, policy, sampler
...
def multi_head_attention_forward(
    ...
    # shape of q/k/v: [B * num_heads, N, C]
    q = q.view(B, num_heads, N, head_dim)  # [B, H, N, C]
    q = q * policy.unsqueeze(1)
    q = q.view(-1, N, head_dim)
    
    k = k.view(B, num_heads, N, head_dim)  # [B, H, N, C]
    k = k * policy.unsqueeze(1)
    k = k.view(-1, N, head_dim)

    v = v.view(B, num_heads, N, head_dim)  # [B, H, N, C]
    v = v * policy.unsqueeze(1)
    v = v.view(-1, N, head_dim)
    ...
    #
    attn_output, attn_output_weights, selected_x, policy, sampler = _scaled_dot_product_attention(
        q, 
        k, 
        v, 
        policy,
    ...
    attn_output = attn_output * policy.transpose(0, 1)

    if need_weights:
        # average attention weights over heads
        attn_output_weights = attn_output_weights.view(bsz, num_heads, tgt_len, src_len)
        return attn_output, attn_output_weights.sum(dim=1) / num_heads, selected_x, policy, sampler
    else:
        return attn_output, None, selected_x, policy, sampler

class AdaptiveTokenSampler(nn.MultiheadAttention):
    def __init__(self, embed_dim, num_heads, dropout=0, bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, batch_first=False, device=None, dtype=None, drop_tokens=False,) -> None:
    ...
    Outputs:
        ...
        if not self._qkv_same_embed_dim:
            attn_output, attn_output_weights, selected_x, policy, sampler = multi_head_attention_forward(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias,
                policy, n_tokens, raw_x, n_ref_tokens,
            ...
            attn_output, attn_output_weights, selected_x, policy, sampler = multi_head_attention_forward(
                ...
                drop_tokens=self.drop_tokens,)
        if self.batch_first:
            return attn_output.transpose(1, 0), attn_output_weights, selected_x, policy, sampler
        else:
            return attn_output, attn_output_weights, selected_x, policy, sampler
