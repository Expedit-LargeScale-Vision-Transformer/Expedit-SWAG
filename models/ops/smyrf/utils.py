...
class XBOXPLUS(AsymmetricTransform):
    def set_norms(self, queries, keys):
        q_norms = queries.norm(p=2, dim=-1).unsqueeze(-1)
        MQ = torch.max(q_norms, dim=1).values.unsqueeze(-1)

        k_norms = keys.norm(p=2, dim=-1).unsqueeze(-1)
        MK = torch.max(k_norms, dim=1).values.unsqueeze(-1)
        return q_norms, k_norms, MQ, MK

    def K(self, x, k_norms, MK, MQ):
        ext = torch.sqrt(MQ**2 + MK**2 - k_norms**2)
        zero = torch.tensor(0.0, device=x.device).repeat(x.shape[:-1], 1).unsqueeze(-1)
        return torch.cat((x, ext, zero), -1)

    def Q(self, x, q_norms, MK, MQ):
        ext = torch.sqrt(MQ**2 + MK**2 - q_norms**2)
        zero = torch.tensor(0.0, device=x.device).repeat(x.shape[:-1], 1).unsqueeze(-1)
